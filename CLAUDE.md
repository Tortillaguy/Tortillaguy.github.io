...
### 02 ART & MOTION
Six pieces in an asymmetric mosaic (Letting Go, Pyro Demo, Hardstyle Visuals, Oblivion V2, Light It Up, Crystals). Each card is two lines: the name, then the `art.ts` tags rendered in natural case as the card's blurb (`Octane · Blender · EmberGen`) — there is no separate right-aligned mono tags line, and no prose provenance note (`mosaicNote` was removed from `artCaseStudies.ts`). Cards link INTERNALLY to `/art/<slug>` case studies (no `target="_blank"`) — the outbound Instagram/Spotify/SoundCloud link now lives on the case-study page's WATCH THE PIECE panel, so a visitor lands on owned content first. All 12 entries in `art.ts` have a page; only these six are surfaced in the mosaic, the other six are reachable via prev/next.

### 03 ENGINEERING
Centered quote + credibility rows (company/role left, green years + tags right) linking to `/work/*` case studies. Rows with an optional `image` in `engineering.ts` render that asset as a background band: full row width on mobile, a `left-[36.4%] w-[40%]` band in the empty gutter on desktop. The horizontal vignette is a CSS `mask-image` (NOT a gradient overlay — the row's `hover:bg-encore-surface` has to show through the faded edges), and brightness is deliberately asymmetric — `0.22` on mobile, where text sits on top of the image, `0.5` on desktop, where it doesn't. All eight rows carry an asset. TwelveLabs (`twelvelabs.png`) and First Resonance (`first-resonance.png`) are crops of the companies' own public marketing imagery — the Rodeo launch key art and an ION Factory OS dashboard shot — since neither case study ships a first-party photo. The band flatters near-black sources with a centred subject — `twelvelabs.png` is the reference (already ~3.8:1, background at the page's own black, so the vignette disappears and the subject reads as floating in the row) — and punishes light, text-dense screenshots: `first-resonance.png` and `infinite-reality.webp` are the two weak ones, sliced mid-glyph and lighter than the rows around them. Linked rows carry an always-visible green `→` (far right; nudges right on hover while the company name lerps to accent). As of Aug 2026 every row is linked — Coachella AR was the last unlinked row and now points at `/work/coachella`. Framing: "the VJ who builds the software behind the show."
